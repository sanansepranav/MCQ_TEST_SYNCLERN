...
const TakeCodingTest = () => {
  ...
  const handleCodingCameraViolation = useCallback(async (violationType, description) => {
    console.log('CODING CAMERA VIOLATION:', violationType, description);
    violationsRef.current = [
      ...violationsRef.current,
      {
        type: violationType,
    ...
    if (violationType === 'camera-cheating-detected') {
      await autoSubmitCoding('camera-cheating-detected');
    }
  }, [mcqResultId, fromCombinedMCQ]);
        ...
        <div className="animate-fade-in" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: 16, padding: 32, maxWidth: 540, width: '100%' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 14, marginBottom: 28 }}>
            <div style={{ background: 'var(--bg-hover)', border: '1px solid var(--border-color)', padding: 16, borderRadius: 12 }}>
              <p style={{ fontSize: '11px', fontWeight: '600', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '4px' }}>DURATION</p>
              <p style={{ fontSize: '26px', fontWeight: '800', fontFamily: 'Sora, sans-serif', color: 'var(--text-primary)', margin: 0 }}>{durationMins}</p>
              <p style={{ fontSize: '12px', color: 'var(--text-secondary)', margin: 0 }}>Minutes</p>
            </div>
            <div style={{ background: 'var(--bg-hover)', border: '1px solid var(--border-color)', padding: 16, borderRadius: 12 }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>PROBLEMS</span>
              <p style={{ fontSize: 26, fontWeight: 800, color: 'var(--text-primary)', margin: '4px 0 0', fontFamily: "'Sora', sans-serif" }}>{problems.length}</p>
              <p style={{ fontSize: '12px', color: 'var(--text-secondary)', margin: 0 }}>Challenges</p>
            </div>
            <div style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.25)', padding: 16, borderRadius: 12 }}>
              ...
              <p style={{ fontSize: 18, fontWeight: 800, color: 'var(--accent-blue)', margin: '6px 0 0', fontFamily: "'Sora', sans-serif" }}>AI Webcam ON</p>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 120px)' }}>
      ...
      <WebcamProctor
        isActive={testStarted && !submitting}
        onViolation={handleCodingCameraViolation}
        maxWarnings={6}
